...
//返回值:0,正常
//    其他,失败
uint8_t MPU6050_DMP_Get_Data_euler(float *pitch,float *roll,float *yaw)//（5MS  200HZ调用一下，给 DEFAULT_MPU_HZ 频率保持一致，或者int中断引脚读取）
...
//返回值:0,正常
//    其他,失败
uint8_t MPU6050_DMP_Get_Data_quaternion(float *Q0,float *Q1,float *Q2, float *Q3)//（5MS  200HZ调用一下，给 DEFAULT_MPU_HZ 频率保持一致，或者int中断引脚读取）
{                              //  （此处在主函数中一直调用，在中断函数里更新 频率未变）
    float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
    unsigned long sensor_timestamp;
    short gyro[3], accel[3], sensors;
